...
- The Pydantic schema for validating API requests when creating a enphase.
...
class enphase(Base):
    """
    SQLAlchemy ORM model representing a enphase record.
    ...
    def __repr__(self):
        """
        Returns a string representation of the enphase instance.

        Example:
            <enphase(id=1, username='johndoe', email='john@example.com')>
        """
        return f"<enphase(id={self.id}, username='{self.username}', email='{self.email}')>"


class enphaseCreate(BaseModel):
    """
    Pydantic schema for creating a new enphase.
